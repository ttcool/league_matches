...
def get_good_name_from_text(context):
    try:
        ...
        for i in results1:
            #print(i)
            good_param.append((i.get('主体参数'), i.get('主体参数.1')))
...
def get_good(driver):
    try:
        ...
        for good in good_list:
            ...
            # 商品评论数
            good_prd_evaluate = good.find_element_by_css_selector(
                '.res-info .evaluate-old .info-evaluate a').text
            # 商品价格
            ...
            if get_good_name_from_text(context):
                ...
                with open('suning_bingxiang_fail.csv', 'a', newline="", encoding='utf-8') as f:
                    csv_writer = csv.writer(f, delimiter='|')
                    csv_writer.writerow([good_url,good_prd_id])

            if good_content:
                import json
                with open('suning_bingxiang_success.csv', 'a', newline="", encoding='utf-8') as f:
                    csv_writer = csv.writer(f, delimiter='|')
